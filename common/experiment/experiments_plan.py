...
class ExperimentsPlan:
    ...
    def __create_experiment_configurations_for_base_config(self, base_config: dict, options: dict) -> List[dict]:
        if len(options) == 0:
            ...
            self.__format_outputs_dir(config)
            return [config]
    ...
    def __format_experiment_name(self, config: dict):
        ...
        for ch in self.ESCAPE_CHARACTERS:
            formatted_experiment_name = formatted_experiment_name.replace(ch, "-")
    ...
    def __format_outputs_dir(self, config: dict):
        outputs_dir = config.get("outputs_dir")
        if not outputs_dir:
            return

        formatted_outputs_dir = outputs_dir.format(**config)

        for ch in self.ESCAPE_CHARACTERS:
            formatted_outputs_dir = formatted_outputs_dir.replace(ch, "-")

        for ch in self.REMOVE_CHARACTERS:
            formatted_outputs_dir = formatted_outputs_dir.replace(ch, "")

        config["outputs_dir"] = formatted_outputs_dir
