...
class Tuner:
    ...
    def __multiprocess_run_search(self, configs_seq, skip: int = 0, repetitions: int = 1, score_reduction: str = "mean") -> TuneResult:
        ...
        with multiprocessing.Pool(processes=self.num_parallel, initializer=_exp_worker_init, initargs=[gpu_ids_queue, log_queue]) as pool:
            config_results = pool.starmap(func=_exp_worker, iterable=workers_arguments_list, chunksize=1)
    ...
    def __create_workers_arguments_list(self, configs_seq, skip: int, repetitions: int = 1, score_reduction: str = "mean") -> List[Tuple]:
        arguments_list = []
        for i, config in enumerate(itertools.islice(configs_seq, skip, None)):
            config_index = skip + i
            arguments_list.append((self.experiment, config, self.context, config_index, len(configs_seq), self.manage_gpu,
                                   self.largest, repetitions, score_reduction))

        return arguments_list


def _exp_worker(experiment: Experiment, config: dict, context: dict, config_index: int, num_configs: int, manage_gpu: bool,
                largest: bool = True, repetitions: int = 1, score_reduction: str = "mean") -> ConfigResult:
    config = config.copy()
    ...
    config_result = ConfigResult(config, score_reduction=score_reduction, largest=largest)
    try:
        _exp_worker.log_queue.put((logging.INFO,
                                   f"PID - {multiprocessing.current_process().pid} - "
                                   f"{_create_start_config_experiment_log_message(config_index, num_configs, repetitions, config)}"))

        for r in range(repetitions):
            _exp_worker.log_queue.put((logging.INFO,
                                       f"PID - {multiprocessing.current_process().pid} - "
                                       f"{_create_start_repetition_log_message(config_index, num_configs, r, repetitions)}"))

            experiment_result = experiment.run(config, context=context)
            config_result.add_experiment_result(experiment_result)

            _exp_worker.log_queue.put((logging.INFO,
                                       f"PID - {multiprocessing.current_process().pid} - "
                                       f"{_create_finish_repetition_log_message(config_index, num_configs, r, repetitions, experiment_result)}"))

        largest_log_str = "maximal" if largest else "minimal"
        fin_conf_exp_log = (logging.INFO,
                            f"PID - {multiprocessing.current_process().pid} - "
                            f"{_create_finish_config_experiment_log_message(config_index, num_configs, repetitions, largest_log_str, config_result)}")
