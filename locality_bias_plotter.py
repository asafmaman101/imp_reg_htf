...
def __populate_plot(ax, experiment_group_plot_infos, args):
    ...
    for i, exp_group_plot_info in enumerate(experiment_group_plot_infos):
        ...
        if args.x_axis_ticks:
            if args.use_x_axis_ticks_values:
                ax.set_xticks(list(map(int, args.x_axis_ticks)))
            ax.set_xticklabels(args.x_axis_ticks)

        if args.y_axis_ticks:
            if args.use_y_axis_ticks_values:
                ax.set_yticks(list(map(float, args.y_axis_ticks)))
            ax.set_yticklabels(args.y_axis_ticks)

        ax.yaxis.set_major_formatter(StrMethodFormatter(args.y_axis_format_str))
    ...
    ax.set_title(args.plot_title, fontsize=TITLE_SIZE, pad=10)
...
def __create_plot_from_experiment_group_plot_infos(experiment_group_plot_infos, args):
    ...
    plt.tight_layout()
    os.makedirs(os.path.dirname(args.save_plot_to), exist_ok=True)
...
def main():
    ...
    p.add_argument("--error_bars_opacity", type=float, default=1, help="Chose alpha value for the opacity of the error bars.")
    ...
    p.add_argument("--x_axis_ticks", type=str, nargs="+", default=[],
                   help="List of values to be placed as x ticks in case that the existing values do not fit the "
                        "required formatting.")
    p.add_argument("--use_x_axis_ticks_values", action="store_true", default=False,
                   help="Whether to use the values passed in x_axis_ticks.")
    p.add_argument("--y_axis_ticks", type=str, nargs="+", default=[],
                   help="List of values to be placed as y ticks in case that the existing values do not fit the "
                        "required formatting.")
    p.add_argument("--use_y_axis_ticks_values", action="store_true", default=False,
                   help="Whether to use the values passed in y_axis_ticks.")
    p.add_argument("--y_axis_format_str", type=str, default='', help="Specify .format type string to format the y "
                                                                     "axis ticks.")
